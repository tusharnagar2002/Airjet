...
if(isset($_POST['login_but'])) {
    ...
    if($row = mysqli_fetch_assoc($result)) {        
        ...
        if($pwd_check == false) {
            header('Location:\bootstrap\admin\admin-sign-in.php?error=wrongpwd');
            exit();
        }
            ...
            header('Location:\airjet\admin\index.php?login=success');
            exit();
        } else {
            header('Location:admin\admin-sign-in.php?error=sqlerror');
            exit();
        }
    } else {
        header('Location:bootstrap\admin\admin-sign-in.php?error=invalidcred');
        exit();
    }
